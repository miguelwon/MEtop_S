/*******************************
*    CalcHEP  3.2*
*******************************/
#include"num_out.h"
#include"num_in.h"
extern FNN F94_out;
static void C94(REAL * C)
{
REAL* V=va_out;
REAL S[9];                                                                  
     
S[0]=V[18]*V[18];
S[1]=V[11]*V[11];
S[2]=V[8]*V[8];
C[9]=+S[2]*(S[1]*(V[17]*(8*V[18]+4*V[17])+4*S[0]));
C[8]=+S[2]*(V[17]*(8*V[17]-16*V[18])+8*S[0]);
C[7]=+S[1]*(V[17]*(8*V[18]+4*V[17])+4*S[0]);
C[6]=+V[17]*(8*V[17]-16*V[18])+8*S[0];
S[3]=V[8]*V[8]*V[8]*V[8];
C[5]=+S[3]*(S[1]*(V[17]*(2*V[18]+V[17])+S[0]));
C[4]=+S[3]*(V[17]*(2*V[17]-4*V[18])+2*S[0]);
C[3]=+S[2]*(S[1]*(V[17]*(4*V[18]+2*V[17])+2*S[0]));
C[2]=+S[2]*(V[17]*(4*V[17]-8*V[18])+4*S[0]);
S[4]=V[14]*V[14];
S[5]=V[2]*V[2]*V[2]*V[2];
C[1]=+9*S[4]*S[5];
S[6]=V[33]*V[33];
S[7]=V[15]*V[15];
S[8]=V[1]*V[1]*V[1]*V[1];
C[0]=+32*S[6]*S[7]*S[8];
}
REAL F94_out(double GG,REAL*DP,REAL*Q0,REAL*Q1,REAL*Q2)
{
/*
         d                 d      !  d                 d             
       ==>==@==============>======!==>==============@==>==           
         P1 |              P5     !  P5             |  P1            
            |                     !                 |                
            |              N2     !  N2             |                
           G|P7         /==<======!==<==\          G|-PA             
            1           |  P3     !  P3 |           3                
            |           |         !     |           |                
         U  |  t     W+ |  e2     !  e2 |  W+    t  |  U             
       ==<==@==<==@-2<--@==>======!==>==@--<4-@==<==@==<==           
         P2    P8 |  P9    P4     !  P4   -PC | -PB    P2            
                  |               !           |                      
                  |        B      !  B        |                      
                  \========<======!==<========/                      
                           P6     !  P6                              
*/
REAL N,D,R;
REAL * V=va_out;
static REAL C[10];REAL S[3];REAL tmp[1];                                    
     
if(CalcConst) C94(C);
S[0]=GG*GG*GG*GG;
N=+C[0]*S[0];
S[1]=V[34]*V[34]*V[34]*V[34];
D=+C[1]*S[1];
S[2]=V[34]*V[34];
tmp[0]=+DP[5]*(C[2]*(DP[0]*(DP[0]*(DP[12]*(DP[2]+DP[4]-DP[8]-DP[9])+DP[13]*(
 DP[2]+DP[4]-DP[8]-DP[9])+S[2]*(DP[11]-DP[14])+DP[5]*(DP[14]-DP[11]))+DP[6]*
 (DP[12]*(DP[8]-DP[2]-DP[4]+DP[9])+DP[13]*(DP[8]-DP[2]-DP[4]+DP[9])+S[2]*(
 DP[14]-DP[11])+DP[5]*(DP[11]-DP[14]))+DP[7]*(DP[12]*(DP[1]+DP[3]-DP[8]-
 DP[9])+DP[13]*(DP[1]+DP[3]-DP[8]-DP[9])+S[2]*(DP[10]-DP[14])+DP[5]*(DP[14]-
 DP[10])))+DP[7]*(DP[6]*(DP[12]*(DP[1]+DP[2]+DP[3]+DP[4])+DP[13]*(DP[1]+
 DP[2]+DP[3]+DP[4])+S[2]*(DP[10]+DP[11])+DP[5]*(-DP[10]-DP[11]))+DP[7]*(
 DP[12]*(DP[1]+DP[2]+DP[3]+DP[4])+DP[13]*(DP[1]+DP[2]+DP[3]+DP[4])+S[2]*(
 DP[10]+DP[11])+DP[5]*(-DP[10]-DP[11]))))+DP[0]*(C[3]*(DP[1]*(DP[12]+
 DP[13])+DP[3]*(DP[12]+DP[13])+DP[10]*(S[2]-DP[5]))+C[4]*(DP[14]*(DP[0]-
 DP[6]+DP[7])+DP[11]*(DP[6]-DP[0])-DP[10]*DP[7])-C[5]*DP[10])+DP[7]*(C[3]*(
 DP[8]*(DP[12]+DP[13])+DP[9]*(DP[12]+DP[13])+DP[14]*(S[2]-DP[5]))+C[4]*(
 DP[6]*(-DP[10]-DP[11])+DP[7]*(-DP[10]-DP[11]))-C[5]*DP[14]));
R=+DP[6]*(S[2]*(DP[12]*(DP[0]*(S[2]*(C[6]*(DP[9]*(DP[6]-DP[0]-DP[7])+DP[4]*(
 DP[0]-DP[6])+DP[7]*DP[3])+C[7]*DP[3])+C[8]*(DP[8]*(DP[0]-DP[6]+DP[7])+
 DP[2]*(DP[6]-DP[0])-DP[7]*DP[1])+C[2]*(DP[9]*(DP[0]-DP[6]+DP[7])+DP[4]*(
 DP[6]-DP[0])-DP[7]*DP[3])-C[9]*DP[1]-C[3]*DP[3])+DP[7]*(DP[6]*(S[2]*(C[6]*(
 DP[3]+DP[4]))+C[8]*(-DP[1]-DP[2])+C[2]*(-DP[3]-DP[4]))+DP[7]*(S[2]*(C[6]*(
 DP[3]+DP[4]))+C[8]*(-DP[1]-DP[2])+C[2]*(-DP[3]-DP[4]))+DP[9]*(C[7]*S[2]-
 C[3])-C[9]*DP[8]))+DP[13]*(C[2]*(DP[0]*(DP[8]*(DP[0]-DP[6]+DP[7])+DP[2]*(
 DP[6]-DP[0])-DP[7]*DP[1])+DP[7]*(DP[1]*(-DP[6]-DP[7])+DP[2]*(-DP[6]-
 DP[7])))+C[3]*(-DP[1]*DP[0]-DP[8]*DP[7])))+tmp[0]);
R*=(N/D)*Q2[4]*Q2[2]*Q2[3];
if(gswidth_out ) R=R*Q0[1];
 if(cb_coeff_out)
 {
  cb_coeff_out[0] += (R*9)/(8);
 }
 return R;
}
